(() => {
  ...
  const formatNumber = value => new Intl.NumberFormat('en', { notation: value >= 10000 ? 'compact' : 'standard' }).format(value);

  Promise.all([
    fetch(repo, { headers: { Accept: 'application/vnd.github+json' } }).then(response => { if (!response.ok) throw new Error('Repository request failed'); return response.json(); }),
    fetch(`${repo}/releases`, { headers: { Accept: 'application/vnd.github+json' } }).then(response => { if (!response.ok) throw new Error('Releases request failed'); return response.json(); })
  ]).then(([repository, releases]) => {
    setText('stars', formatNumber(repository.stargazers_count));
